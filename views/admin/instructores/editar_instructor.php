...
if (isset($_GET['id'])) {
    ...
    // Consulta para obtener los datos de la formación seleccionada
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $nombre = $_POST['nombre'];
        $email = $_POST['email'];
        $documento = $_POST['documento']; // Asegúrate de tener el valor del documento del formulario o de otra fuente.

        // Verificar si el valor de `ficha` existe en la tabla `formacion`
        ...
        $fichaExiste = $verificarFichaQuery->fetchColumn(); // Obtiene el recuento de registros

        // Si `ficha` existe en la tabla `formacion`
        if ($fichaExiste > 0) {
            // Actualizar los datos en la tabla `usuario` excepto la ficha
            ...
            $updateUsuarioQuery->bindParam(':documento', $id);
            ...
            $updateFichaQuery->bindParam(':documento', $id);
            ...
            // Si `ficha` no existe en la tabla `formacion`
...
if (isset($_SESSION['documento'])) {
    ...
    $usuarioQuery->bindParam(':documento', $id);
                                ...
                                <div class="card-body">
                                    ...
                                    <form id="formulario" method="post" action="" autocomplete="off">
                                        <div class="formulario__grupo" id="grupo__documento">
                                            ...
                                            <p class="formulario__input-error">El documento debe contener de 6 a 11 dígitos.</p>
                                        </div>

                                        <div class="formulario__grupo" id="grupo__nombre">
                                            <label for="nombre" class="formulario__label">Nombres y apellidos:</label>
                                            <input type="text" class="formulario__input" id="nombre" name="nombre" value="<?= $nombre_usuario ?>" required>
