...
<body class="main-layout in_page">
                            ...
                            <div class="collapse navbar-collapse" id="navbarsExample04">
                                <ul class="navbar-nav mr-auto">
                                    ...
                                    <li class="nav-item active">
                                        <a class="nav-link" href="#">tus prestamos a devolver</a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link" href="lista_devoluciones.php">tus devoluciones</a>
                                    </li>
                                </ul>
                            </div>
                        </nav>
    ...
    <div class="container mt-3">
        <h2 style="text-transform:uppercase;">tus prestamos a devolver</h2>
                ...
                <table class="table table-striped table-bordered table-hover">
                    ...
                    <tbody>
                        <?php foreach ($asigna as $usua) {
                            // Realizar una consulta adicional para obtener los datos relacionados con cada préstamo
                            $usuar = $conectar->prepare("SELECT * FROM detalle_pres WHERE id_prestamo = ?");
                            $usuar->execute([$usua["id_prestamo"]]);
                            $asignar = $usuar->fetch(PDO::FETCH_ASSOC);
                        ?>
                            <tr>
                                ...
                                <td>
                                    <a href="detalle_prestamo.php?id=<?= $usua["id_prestamo"] ?>" class="btn btn-primary">detalles prestamo</a>
                                    <?php if ($asignar) : ?>
                                        <a href="#" class="btn btn-danger" onclick="return confirmAndOpenPopup('devolver_prestamo.php?id=<?= $usua["id_prestamo"] ?>&id_detalle=<?= $asignar["id_de_pres"] ?>', '¿Estás seguro de devolver estas herramientas?')">Devolver</a>
                                    <?php else : ?>
                                        <a class="btn btn-danger disabled">Devolver</a>
                                    <?php endif; ?>
                                </td>
                            </tr>
                        <?php } ?>
