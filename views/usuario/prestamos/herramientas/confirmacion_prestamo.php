...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["ids"])) {
    ...
    if (isset($_SESSION['documento'])) {
        ...
        if ($row) {
            ...
            if ($total_filas_activas >= 3) {
                echo '<script>alert("Tienes 3 o más reportes, no puedes hacer más préstamos");</script>';
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["ids_seleccionados"])) {
    ...
    if ($suma_cantidades > 5) {
        echo '<script>alert("No se puede hacer el préstamo. Solo se permiten hasta 5 herramientas en total.");</script>';
        echo '<script>window.location= "lista_herramientas.php"</script>';
                    ...
                    <tbody>
                        <?php foreach ($asigna as $usua) { ?>
                            <tr>
                                ...
                                <td><?= $usua["estado"] ?></td>
                                <td><input type="number" name="cantidad[]" min="0" max="<?= $usua['stock'] ?>" value="0" class="form-control mt-2" placeholder="Cantidad a prestar">
                                </td>
